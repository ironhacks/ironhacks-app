 ...
 */
  ...
  const uniqueArray = function uniqueArray(arr) {
    const result = [];
 ...
 */
  ...
  const toArray = function toArray(nodeList) {
 ...
 */
  ...
  const formatInputOptions = function formatInputOptions(inputOptions) {
    const result = [];
 ...
 */
  const adaptInputValidator = function adaptInputValidator(legacyValidator) {
  ...
  const prefix = function prefix(items) {
    const result = {};
  ...
  const addOrRemoveClass = function addOrRemoveClass(target, classList, add) {
    if (!target || !classList) {
      return;
    }
  ...
  const addClass = function addClass(target, classList) {
    addOrRemoveClass(target, classList, true);
  };
  const removeClass = function removeClass(target, classList) {
    addOrRemoveClass(target, classList, false);
  };
  const getChildByClass = function getChildByClass(elem, className) {
  ...
  const contains = function contains(haystack, needle) {
  ...
  const getIcons = function getIcons() {
  ...
  const getButtonsWrapper = function getButtonsWrapper() {
  ...
  const getFocusableElements = function getFocusableElements() {
    ...
    const otherFocusableElements = toArray(getPopup().querySelectorAll('a[href], area[href], input:not([disabled]), select:not([disabled]), textarea:not([disabled]), button:not([disabled]), iframe, object, embed, [tabindex="0"], [contenteditable], audio[controls], video[controls]')).filter(function(el) {
 ...
 */
  ...
  // Clean up the old popup if it exists
    ...
    const targetElement = typeof params.target === 'string' ? document.querySelector(params.target) : params.target;
    targetElement.appendChild(container);
    const popup = getPopup();
    const content = getContent();
    const input = getChildByClass(content, swalClasses.input);
    const file = getChildByClass(content, swalClasses.file);
    const range = content.querySelector('.'.concat(swalClasses.range, ' input'));
    const rangeOutput = content.querySelector('.'.concat(swalClasses.range, ' output'));
    const select = getChildByClass(content, swalClasses.select);
    const checkbox = content.querySelector('.'.concat(swalClasses.checkbox, ' input'));
    const textarea = getChildByClass(content, swalClasses.textarea); // a11y
    ...
    const resetValidationMessage = function resetValidationMessage(e) {
  ...
  const renderActions = function renderActions(params) {
  ...
  const renderContent = function renderContent(params) {
    const content = getContent().querySelector('#' + swalClasses.content); // Content as HTML
  ...
  const renderIcon = function renderIcon(params) {
    ...
    if (params.type) {
      if (Object.keys(iconTypes).indexOf(params.type) !== -1) {
        const icon = Swal.getPopup().querySelector('.'.concat(swalClasses.icon, '.').concat(iconTypes[params.type]));
  ...
  const renderImage = function renderImage(params) {
  ...
  const renderProgressSteps = function renderProgressSteps(params) {
    const progressStepsContainer = getProgressSteps();
    const currentProgressStep = parseInt(params.currentProgressStep === null ? Swal.getQueueStep() : params.currentProgressStep, 10);
  ...
  const renderTitle = function renderTitle(params) {
  ...
  const fixScrollbar = function fixScrollbar() {
  ...
  const undoScrollbar = function undoScrollbar() {
    if (states.previousBodyPadding !== null) {
  ...
  const fixVerticalPositionIE = function fixVerticalPositionIE() {
    const container = getContainer();
    const popup = getPopup();
  ...
  const undoIEfix = function undoIEfix() {
    if (typeof window !== 'undefined' && isIE11()) {
  ...
  const globalState = {};
  const restoreActiveElement = function restoreActiveElement() {
 ...
 */
  ...
  const close = function close(onClose, onAfterClose) {
    const container = getContainer();
    const popup = getPopup();
    ...
    const removePopupAndResetState = function removePopupAndResetState() {
      if (!isToast()) {
 ...
 */
  ...
  const isVisible$1 = function isVisible() {
    return !!getPopup();
  };
  /*
 ...
 */
  function withNoNewKeyword(ParentSwal) {
    const NoNewKeywordSwal = function NoNewKeywordSwal() {
      for (var _len = arguments.length, args = new Array(_len), _key = 0; _key < _len; _key++) {
        args[_key] = arguments[_key];
      }
  ...
  const deprecatedParams = ['useRejections', 'expectRejections', 'extraParams'];
 ...
 */
  ...
  const queue = function queue(steps) {
    const swal = this;
    currentSteps = steps;

    const resetQueue = function resetQueue() {
      currentSteps = [];
 ...
 */
  ...
  const getQueueStep = function getQueueStep() {
 ...
 */
  ...
  const insertQueueStep = function insertQueueStep(step, index) {
 ...
 */
  ...
  const deleteQueueStep = function deleteQueueStep(index) {
 ...
 */
  ...
  const increaseTimer = function increaseTimer(n) {
 ...
 */
  ...
  const staticMethods = Object.freeze({
  ...
  /* istanbul ignore next */
  const _Symbol = typeof Symbol === 'function' ? Symbol : function() {
    let idCounter = 0;
  ...
  const WeakMap$1 = typeof WeakMap === 'function' ? WeakMap : function(s, dP, hOP) {
    function WeakMap() {
      dP(this, s, {
 ...
 */
  ...
  function setProgressSteps(progressSteps) {
    ...
    const updatedParams = _extends({}, innerParams, {
  ...
  const Timer = function Timer(callback, delay) {
 ...
 */
  ...
  // Use default `inputValidator` for supported input types if not provided
    ...
    const targetElement = typeof params.target === 'string' ? document.querySelector(params.target) : params.target; // If the model target has changed, refresh the popup
    ...
    if (params.grow && typeof params.grow === 'string') {
      const growClass = 'grow-' + params.grow;
 ...
 */
  ...
  function _main(userParams) {
    ...
    const innerParams = _extends({}, defaultParams, userParams);
    ...
    // functions to handle all resolving/rejecting/settling
      const succeedWith = function succeedWith(value) {
      ...
      const errorWith = function errorWith(error$$1) {
        constructor.closePopup(innerParams.onClose, innerParams.onAfterClose);
        reject(error$$1);
      }; // Close on timer
      ...
      const confirm = function confirm(value) {
      ...
      const onButtonEvent = function onButtonEvent(e) {
        const target = e.target;
        const confirmButton = domCache.confirmButton;
        const cancelButton = domCache.cancelButton;
      ...
      const setFocus = function setFocus(index, increment) {
        const focusableElements = getFocusableElements(innerParams.focusCancel); // search for visible elements and select the next possible match

        for (let _i = 0; _i < focusableElements.length; _i++) {
      ...
      const keydownHandler = function keydownHandler(e, innerParams) {
        ...
        if (e.key === 'Enter' && !e.isComposing) {
          ...
          const focusableElements = getFocusableElements(innerParams.focusCancel);
          let btnIndex = -1;

          for (let _i2 = 0; _i2 < focusableElements.length; _i2++) {
      ...
      const setInputPlaceholder = function setInputPlaceholder(input) {
      ...
      for (let _i3 = 0; _i3 < inputTypes.length; _i3++) {
        const inputClass = swalClasses[inputTypes[_i3]];
        const inputContainer = getChildByClass(domCache.content, inputClass);
        input = _this.getInput(inputTypes[_i3]); // set attributes

        if (input) {
          for (const j in input.attributes) {
          ...
          for (const attr in innerParams.inputAttributes) {
      ...
      switch (innerParams.input) {
        ...
        case 'range':
        {
          const range = getChildByClass(domCache.content, swalClasses.range);
        ...
        case 'select':
        {
          const select = getChildByClass(domCache.content, swalClasses.select);
          select.innerHTML = '';
        ...
        case 'radio':
        {
          const radio = getChildByClass(domCache.content, swalClasses.radio);
          radio.innerHTML = '';
        ...
        case 'checkbox':
        {
          const checkbox = getChildByClass(domCache.content, swalClasses.checkbox);

          const checkboxInput = _this.getInput('checkbox');
        ...
        case 'textarea':
        {
          const textarea = getChildByClass(domCache.content, swalClasses.textarea);
...
typeof document!=='undefined'&&function(e, t) {
  const n=e.createElement('style'); if (e.getElementsByTagName('head')[0].appendChild(n), n.styleSheet){n.styleSheet.disabled||(n.styleSheet.cssText=t);} else {
    try {
      n.innerHTML=t;
    } catch (e) {
      n.innerText=t;
