...
class PostView extends Component {
  ...
  deletePost = () => {
    Swal.fire({
      ...
      if (result.value) {
        let commentDocs = this.props.postRef.collection('comments').get()

        Promise.resolve(commentDocs).then((docs) => {
          if (docs.empty) {
      ...
      <div className="post depth-1 mb-2 p-5 relative">
        ...
        {this.props.data.author === this.state.user.uid && (
          <div className="post_controls">
            ...
            />
            <MaterialDesignIcon name="delete" iconClass="btn" onClick={this.deletePost} />
          </div>
        )}
