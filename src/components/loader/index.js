function Spinner() {
  return <div className="loader_spinner" />
}
...
function SpinnerContainer({ color, small, children }) {
  ...
  return <div class={loaderSize}>{children}</div>
}
...
const Loader = (props) => {
  ...
  return (
    <div className={loaderSize}>
      <div className="loader_spinner" style={{ color: props.color ? props.color : '' }} />

      {props.status && <div className="loader_status">{props.status}</div>}
    </div>
  )
}
