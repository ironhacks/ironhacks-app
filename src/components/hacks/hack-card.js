...
const HackCard = ({ slug, thumbImg, name, description, startDate, difficulty }) => {
  return (
    <div className="hack_card">
      <a href={`/hacks/${slug}`}>
        <div className="flex">
          <div className="hack_card__header">
            {thumbImg && <img className="card_img" src={thumbImg} alt={`${name} Thumbnail`} />}
          </div>

          <div className="hack_card__body">
            <h3 className="card_title font-bold">{name}</h3>

            {description && <p className="card_description">{description}</p>}

            <div className="card_meta">
              {startDate && <span className="mr-3">Opening Date: {formatDate(startDate)}</span>}

              {difficulty && <span>Difficulty: {difficulty.label}</span>}
            </div>
          </div>
        </div>
      </a>
...
const PreviousHackCard = ({ thumbImg, name, description, startDate, difficulty }) => {
  return (
    <div className="hack_card">
      <div className="flex">
        <div className="hack_card__header">
...
export { HackCard, HackSignupCard, PreviousHackCard }
