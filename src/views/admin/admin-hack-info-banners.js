      ...
      <>
        <Section sectionClass="py-2">
          <h2 className="h3 font-bold">
            {`${this.props.hackName} Info Banners`}
          </h2>
        </Section>

        <Section sectionClass="mb-3">
          <InfoBannerForm
            onCreate={this.createInfoBanner}
        ...
        <Section>
          ...
          {this.state.banners.map((item, index)=>(
            <InfoBannerPreview
              ...
              active={item.active}
              onToggleActive={(bannerId, active)=>{this.updateInfoBannerActive(index, bannerId, active)}}
              onDeleteInfoBanner={(bannerId)=>{this.deleteInfoBanner(index, bannerId)}}
            />
          ))}
          </div>
        </Section>
        ...
        <Section>
          <p>
            <strong>Note:</strong> If more than one banner is active at one time, only the one with the earliest starting date will be displayed.
          </p>

          <p>
            List of available emojis list: <a href="https://github.com/showdownjs/showdown/wiki/Emojis" className="text-underline" target="_blank">showdownjs/showdown/wiki/Emojis</a>
          </p>
        </Section>
      </>
    )
