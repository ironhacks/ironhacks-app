...
class AdminHackResults extends Component {
  ...
  getSubmissions = async () => {
    ...
    submissions.sort((a, b) => {
      return fire2Ms(data[a].deadline) - fire2Ms(data[b].deadline)
    })
    ...
    return (
      <>
        <Section sectionClass="py-2">
          <h2 className="h3 font-bold">{`${this.props.hackName} Submission Results`}</h2>

          <h3 className="h4 fm-1 font-bold">Manage Results:</h3>

          <div className="bg-grey-lt2 py-1">
            <ul className="list my-0">
              {this.state.submissions.map((item, index) => (
                <li key={index}>
                  <Link to={`results/${item}`}>{item}</Link>
          ...
          <h2 className="h3 pt-2">Results Dashboard Document</h2>

          <p>Default information related to the results data displayed above in the dashboard</p>
          ...
          >
            <Button primary width="150px" margin="0 0 0 15px" onClick={this.updateContent}>
              Publish
            </Button>
