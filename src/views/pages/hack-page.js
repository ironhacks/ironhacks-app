...
const HackNavSection = ({ hackDisplayOptions, hackId, hackSlug }) => {
  return (
    <Row>
      <HackNav hackDisplayOptions={hackDisplayOptions} hackId={hackId} hackSlug={hackSlug} />
    </Row>
  )
}
...
class HackPage extends Component {
  ...
  getHack = async (hackSlug) => {
    let hacks = await window.firebase
      .firestore()
      .collection('hacks')
    ...
    if (hacks.docs[0].exists) {
      ...
      let cohortSettingsDoc = await window.firebase
        .firestore()
        .collection('hacks')
        .doc(hackId)
        .collection('registration')
    ...
    return (
      ...
      >
        ...
        <Section sectionClass="mb-0 bg-grey-dk4 cl-white">
          <Row>
            <HackPageBreadCrumbs hackSlug={this.hackSlug} hackName={this.state.name} />
          </Row>
        </Section>
        ...
        {this.state.upcomingEvent && (
          ...
          >
            <span>
              Upcoming: <strong>{this.state.upcomingEvent.name}</strong>
            </span>
            ...
            >
              <CountdownTimer endTime={this.state.upcomingEvent.date} />
            </div>
          </div>
        )}
        ...
        <Switch>
          ...
          <Route exact path="/hacks/:hackId/calendar">
            <Section>
              <Hack.Calendar data={this.state.extensions} hackId={this.state.hackId} />
            </Section>
          </Route>
