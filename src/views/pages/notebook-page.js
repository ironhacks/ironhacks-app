...
class NotebookPage extends Component {
  constructor(props) {
    ...
    if (found) {
      ...
      this.name = decodeURIComponent(this.path)
        .split('/')
        .pop()
  ...
  componentDidMount() {
    window.firebase.analytics().logEvent('view_notebook', {
      filePath: this.path,
      fileName: this.name,
    })
    ...
    if (!this.props.user) {
      ...
      return (
        ...
        >
          <Section>
            <Row>
              <NotebookViewer title="Notebook Viewer" subtitle={this.name} file={this.path} />
            </Row>
          </Section>
        </Page>
      )
