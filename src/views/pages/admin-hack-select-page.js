      ...
      <a href={`/admin/hacks/${this.props.hackId}/settings`}>
        <div className={'admin_hack_card py-2 px-4'}>
          <h3 className="hack_title font-bold text-center">
            {this.props.name}
            <div className="ml-3 badge font-italic">
              ({this.props.hackId})
            </div>
          </h3>


          <div className="ml-3 badge badge-dark">
...
class AdminHackSelectPage extends Component {
  ...
  getUserCount =  async () => {
    let userSnap = await window.firebase.firestore()
      .collection('users')
      .get()
  ...
  getHacks = async () => {
    ...
    hacks.sort((a,b)=>{ return b.startDate.localeCompare(a.startDate) })
