...
export const getHackByID = async (hackID) => {
  ...
  try {
    const response = await firestore
      .collection(COLLECTIONS.HACKS)
      .doc(hackID)
      .get()
    if (!response.exist) {
      throw new Error('No Hack with that ID!')
    }
...
export const getAdminHackData = async (hackObject, _hackID) => {
  ...
  try {
    const hackAdminData = await firestore
      .collection(COLLECTIONS.ADMIN_HACK_DATA)
      .doc(hackID)
      .get()
...
export const updateWhitelist = async (whitelist, hackID) => {
  ...
  const hackRef = firestore.collection(COLLECTIONS.ADMIN_HACK_DATA).doc(hackID)
...
export const getThreadsByHackId = async (hackID) => {
  var result = []
  let docs = await window.firebase
    .firestore()
...
export async function getHack(hackId) {
  let hack = await window.firebase
    .firestore()
    .collection('hacks')
    .doc(hackId)
  return hack
    ...
    .then((result) => {
      ...
      if (!result.empty) {
        result.docs.forEach((hack) => {
          ...
          hacks.push(Object.assign({ available: true }, hackData, { hackId }, { hackName }))
        })
      }
    })
}
