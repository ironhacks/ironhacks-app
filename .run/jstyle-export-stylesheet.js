      ...
      ImportDeclaration: {
        ...
        exit: function(decl, file) {
          ...
          if (node.source.value.endsWith('.css')) {
            ...
            var value = toTree(
              t,
              cssToJss({ code: fs.readFileSync(mod.src).toString() })['@global']
            ); // due to bugs we cannot use t.valueToNode
            decl.replaceWith(
              t.variableDeclaration('var', [t.variableDeclarator(id, value)])
            );
          } else if (node.source.value.endsWith('.css!')) {
            var mod = requireResolve(
              node.source.value.slice(0, -1),
              path.resolve(file.file.opts.filename)
            );
            var id = t.identifier(node.specifiers[0].local.name);
            var value = toTree(
              t,
              cssToJss({ code: fs.readFileSync(mod.src).toString() })
            ); // due to bugs we cannot use t.valueToNode
